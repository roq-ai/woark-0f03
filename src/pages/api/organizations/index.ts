...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.online_form?.length > 0) {
      const create_online_form = body.online_form;
      body.online_form = {
        create: create_online_form,
      };
    } else {
      delete body.online_form;
    }
    if (body?.seo_option?.length > 0) {
      const create_seo_option = body.seo_option;
      body.seo_option = {
        create: create_seo_option,
      };
    } else {
      delete body.seo_option;
    }
