...
import { getContents, deleteContentById } from 'apiSdk/contents';
...
function ContentListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('content', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/contents/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('content', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/contents/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('content', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/contents/edit/${record.id}`} passHref legacyBehavior>
                          <Button
